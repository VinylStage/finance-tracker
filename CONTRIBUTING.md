...
**진행 상태의 정본은 GitHub 마일스톤이다.** 그래서 다음 둘을 지킨다.

- **이슈에는 마일스톤을 지정한다.** 어느 마일스톤에서 한 일인지 남지 않으면, 나중에 그 마일스톤이 무엇을 했는지 되짚을 수 없다. 어느 마일스톤에도 속하지 않는 일이면 지정하지 않아도 되지만, 그건 예외이지 기본이 아니다
- **마일스톤은 마지막 항목이 닫히면 닫는다.** 완료된 마일스톤이 열린 채 남아 있으면 목록만 봐서는 무엇이 끝났는지 알 수 없다

`.github/workflows/milestone-guard.yml` 이 둘을 본다. GitHub 은 이슈 생성 시 마일스톤을 강제할 수단이 없어서 **막는 대신 보이게 한다**.

| 시점 | 동작 |
|---|---|
| 마일스톤 없이 이슈가 열림 | `needs-milestone` 라벨 부착 |
| 마일스톤 지정 | 라벨 자동 제거 |
| 마일스톤 없이 이슈가 닫힘 | 라벨 유지 + 안내 코멘트 1회 |
| 마일스톤의 마지막 항목이 닫힘 | 그 마일스톤을 자동으로 닫음 |
| 매주 월요일 | 마일스톤 없이 열려 있는 이슈에 라벨을 채우고 건수를 요약에 남김 |

자동 닫힘은 **방금 닫힌 이슈의 마일스톤 하나만** 본다. 저장소 전체를 훑어 일괄로 닫지 않는다 — "열린 이슈 0개" 와 "목표 달성" 은 다르다. 잘못 닫혔으면 마일스톤 페이지에서 다시 열면 된다.

이 규칙은 관행이 아니라 워크플로로 둔다. 이 저장소에서 문서 체크리스트만으로 규율을 유지하려던 시도(#33)는 실패했다.
