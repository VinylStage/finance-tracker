...
**[사이클 2 개정 루브릭 적용]** `AUDIT_FRAMEWORK.md` 판정 규칙 5에 따라 세 지표를
병기한다. 본 감사에서 위협모델 예외로 Pass가 된 항목은 **0건**이다 — A1·A4·A6의
예외 전제가 모두 문서(`docs/ARCHITECTURE.md`)로만 존재하고 코드로 강제되지 않아
규칙 2에 따라 Partial로 판정됐기 때문이다. 따라서 이번 사이클은 Pass 비율과
실질 Pass 비율이 일치한다.

| 지표 | 정의 | R2 값 |
|---|---|---:|
| Pass 비율 | (Pass + Pass(예외)) / 21 | **52.4%** |
| 실질 Pass 비율 | Pass / 21 | **52.4%** |
| 예외 비율 | Pass(예외) / 21 | **0%** |

권고 A4-1(비루프백 바인딩 시 기동 거부)이 반영되면 A1·A4·A6가 `Pass(예외)`로
전환될 수 있다. 그 경우 **Pass 비율은 66.7%로 오르지만 실질 Pass 비율은 52.4%로
유지**되므로, 두 지표를 반드시 함께 읽어야 한다.
