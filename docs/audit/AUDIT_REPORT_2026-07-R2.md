...
|---|---:|---:|---:|---:|
| 2차 (R2, 본 보고서) | 11 | 10 | 0 | **52%** |

1차 감사는 루브릭 21항목에 대한 Pass/Partial/Fail 집계를 본 보고서와 동일한
형식으로 제시하지 않아, **직접 비교 가능한 기준선이 없다.** 본 보고서의
수치를 2차 사이클의 공식 기준선으로 삼고, 3차부터 추이를 추적할 것을 제안한다.

측정 가능한 정량 기준선도 함께 고정한다.
...
| 클라이언트 커버리지 | 0% (테스트 부재) |
| 코드 중복률(전체) | 1.97% |
| CC>10 함수 비율 | 3.4% (8/237), 최대 22 |
| 순환 의존성 | 0건 |
| npm audit High 이상 | 0건 |
| API p95 최댓값 (200k건) | 327.5ms (기준 800ms) |
