...
// 가산한 할부금" 이라고 쓴다.
//
// ── 2026-08-18 재확인: 위 두 인용을 **공개 문서에서 다시 못 찾았다**(#491 D2)
//
// 여신금융협회 소비자지원센터 · 비씨카드 고객센터 · 신한카드 소비자가이드 셋을 봤는데
// 「100원당 수수료」 표도, 「월간 수수료」 표현도 지금은 없다. 세 곳 다 **일할 공식만**
// 싣는다(그 공식 자체는 위와 글자까지 같아 여전히 유효하다).
//
// **결정을 바꾸지 않는다.** 근거가 사라진 것이 곧 결정이 틀렸다는 뜻은 아니고,
// 안내 페이지에서 내려갔거나 회원 약관 PDF 쪽에 남아 있을 수 있다. 그리고 월할을
// 뒷받침하는 실측이 따로 있다 — 실사용 DB 의 할부 한 건에서 저장된 월 청구액이
// 1회차 원금과 정확히 일치한다(`installmentBilling.js:27`).
//
// 다만 **근거 등급은 「공식」 에서 「실측」 으로 내렸다.** 공시 인용에 기대지 않고
// 실측 한 건에 기대는 상태라는 뜻이고, 청구서로 한 번 더 맞추면 다시 올라간다.
//
