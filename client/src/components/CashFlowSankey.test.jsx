...
describe('빈 상태', () => {
  it('그릴 것이 없으면 문구로 알린다', async () => {
    render(<CashFlowSankey rows={[]} income={0} />);
    await act(async () => {});  // 초기 효과가 act 안에서 끝나게 한다
    expect(screen.getByText(/그릴 내역이 없습니다/)).toBeTruthy();
  });

  it('수입만 있고 지출이 없어도 남은 돈은 그린다', async () => {
...
describe('노드와 밴드', () => {
  ...
  it('노드 높이가 비율에 비례한다 — 작은 갈래에 최소 높이를 주지 않는다', async () => {
...
describe('색 제약 (#241 — hue 2개 원칙의 예외 구역)', () => {
  ...
  it('수입 노드는 무채색이다 — 출발점이지 갈래가 아니다', async () => {
    const { container } = render(<CashFlowSankey rows={ROWS} income={INCOME} />);
    await act(async () => {});  // 초기 효과가 act 안에서 끝나게 한다
...
describe('접근성', () => {
  it('차트 내용을 읽을 수 있는 설명이 붙는다', async () => {
    render(<CashFlowSankey rows={ROWS} income={INCOME} />);
    await act(async () => {});  // 초기 효과가 act 안에서 끝나게 한다
