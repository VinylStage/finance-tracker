...
const SKIP_REASONS = {
  ...
  // 해외 조건(#710). **방향을 나눈다** — 한 문구로 뭉치면 둘 중 하나에는 틀린
  // 말이 된다. 그리고 이 둘을 안 넣으면 사유가 폴백으로 떨어져 «적용 조건에
  // 맞지 않아요» 만 나온다. 왜 안 맞는지가 빠지면 사용자는 혜택이 없는 줄 안다 —
  // #688 이 «해당하는 혜택이 없어요» 로 같은 잘못을 저질렀다.
  'overseas-only': '해외 결제에만 붙는 혜택이에요.',
  'domestic-only': '국내 결제에만 붙는 혜택이에요.',
};
