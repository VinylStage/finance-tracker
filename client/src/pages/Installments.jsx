  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold text-gray-100">할부 관리</h1>
        <button
      ...
      <div className="flex items-center justify-between">
          ...
          {STATUS_FILTERS.map(s => (
            <button
              ...
              className={`text-xs px-3 py-1.5 rounded-md transition-colors ${
                filter === s ? 'bg-indigo-600 text-white' : 'text-gray-400 hover:text-gray-200 hover:bg-gray-700'
              }`}
            >
              {s}
            </button>
        ...
        <p className="text-sm text-gray-400">
          이번달 청구 합계: <span className="text-gray-100 font-semibold">{fmt(thisMonthTotal)}</span>
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-gray-500 text-center py-10">할부 내역이 없습니다.</div>
      ) : (
        <div className="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-750">
              <tr className="border-b border-gray-700">
                <th className="text-left px-4 py-3 text-gray-400 font-medium">가맹점</th>
                <th className="text-right px-4 py-3 text-gray-400 font-medium">총액</th>
                <th className="text-right px-4 py-3 text-gray-400 font-medium">월납부</th>
                <th className="text-center px-4 py-3 text-gray-400 font-medium">진행</th>
                <th className="text-center px-4 py-3 text-gray-400 font-medium">잔여</th>
            ...
            <tbody>
              {items.map((it, i) => (
                ...
                >
                  <td className="px-4 py-3 text-gray-100">{it.merchant}</td>
                  <td className="px-4 py-3 text-right text-gray-300 tabular-nums">{fmt(it.total_amount)}</td>
                  <td className="px-4 py-3 text-right text-gray-100 tabular-nums">{fmt(it.monthly_amount)}</td>
                  <td className="px-4 py-3 text-center text-gray-400">{it.billed_months}/{it.months}</td>
                  <td className="px-4 py-3 text-center text-gray-400">
                    {it.remaining_months > 0 ? `${it.remaining_months}개월` : '-'}
                  </td>
                  <td className="px-4 py-3 text-gray-500 text-xs hidden sm:table-cell">{it.payment_method_name || '—'}</td>
                  <td className="px-4 py-3 text-center">
                    <span className={`text-xs font-medium ${it.status === '진행중' ? 'text-indigo-300' : 'text-gray-500'}`}>
                      {it.status}
                    </span>
                  </td>
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-gray-800 rounded-xl border border-gray-700 p-5 space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div>
          <label className="block text-xs text-gray-400 mb-1">청구 시작월 *</label>
