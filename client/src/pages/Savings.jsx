          ...
          <table className="w-full text-sm">
            <thead className="bg-surface-muted">
              <tr className="border-b border-line">
                <th className="text-left px-4 py-3 text-ink-subtle font-medium">상품명</th>
                <th className="text-right px-4 py-3 text-ink-subtle font-medium">월 납입액</th>
                <th className="text-left px-4 py-3 text-ink-subtle font-medium hidden sm:table-cell">시작일</th>
                <th className="text-left px-4 py-3 text-ink-subtle font-medium hidden sm:table-cell">만기일</th>
                <th className="text-left px-4 py-3 text-ink-subtle font-medium">목표 진행</th>
                <th className="text-right px-4 py-3 text-ink-subtle font-medium">예상 수령액</th>
                <th className="text-center px-4 py-3 text-ink-subtle font-medium">상태</th>
                <th className="px-4 py-3"></th>
              </tr>
            </thead>
            <tbody>
              {items.map((s, i) => (
                <tr key={s.id} className={`border-b border-line-soft hover:bg-surface-muted transition-colors ${i % 2 === 0 ? '' : 'bg-surface-muted/50'}`}>
                  <td className="px-4 py-3 text-ink">{s.name}</td>
                  <td className="px-4 py-3 text-right text-ink-muted tabular-nums">{fmt(s.monthly_contribution)}</td>
                  <td className="px-4 py-3 text-ink-subtle text-xs hidden sm:table-cell">{s.start_date}</td>
                  <td className="px-4 py-3 text-ink-subtle text-xs hidden sm:table-cell">{s.maturity_date || '—'}</td>
                  <td className="px-4 py-3">
                    <SavingsGoalBar product={s} today={today} />
                  </td>
                  <td className="px-4 py-3 text-right text-accent font-medium tabular-nums">{s.expected_payout ? fmt(s.expected_payout) : '—'}</td>
                  <td className="px-4 py-3 text-center">
                    <span className={`text-xs font-medium ${s.status === '진행중' ? 'text-accent' : 'text-ink-faint'}`}>{s.status}</span>
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex gap-2 justify-end flex-wrap">
                      {s.status === '진행중' && (
                        <button onClick={() => handleMature(s)} className="text-ink-faint hover:text-income transition-colors text-xs">만기처리</button>
                      )}
                      <button onClick={() => { setEditItem(s); setShowForm(true); }} className="text-ink-faint hover:text-accent transition-colors text-xs">수정</button>
                      <button onClick={() => handleDelete(s.id)} className="text-ink-faint hover:text-expense transition-colors text-xs">삭제</button>
                    </div>
                  </td>
                </tr>
              ))}
...
function SavingsForm({ initial, categories, onSave, onCancel }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-surface shadow-card rounded-card border border-line p-5 space-y-4">
      <h2 className="text-sm font-semibold text-ink-muted">{initial ? '상품 수정' : '상품 등록'}</h2>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="sm:col-span-2">
          <label htmlFor="savings-name" className="block text-xs text-ink-subtle mb-1">상품명 *</label>
          <input id="savings-name" type="text" className={inp} value={form.name} onChange={e => set('name', e.target.value)} required />
        </div>
        <div>
          <label htmlFor="savings-monthly-contribution" className="block text-xs text-ink-subtle mb-1">월 납입액 (원) *</label>
        ...
        <div>
          <label htmlFor="savings-start-date" className="block text-xs text-ink-subtle mb-1">시작일 *</label>
        ...
        <div>
          <label htmlFor="savings-maturity-date" className="block text-xs text-ink-subtle mb-1">만기일</label>
        ...
        <div>
          <label htmlFor="savings-expected-payout" className="block text-xs text-ink-subtle mb-1">예상 수령액 (원)</label>
        ...
        <div className="sm:col-span-3">
          <label htmlFor="savings-category" className="block text-xs text-ink-subtle mb-1">저축 카테고리 (만기 시 원금 회수 기록에 사용)</label>
