...
describe('docs/GUIDE.md 실물', () => {
  ...
  // 같은 제목이 두 번 나오면 문서가 같은 기능을 두 번, 대개 다르게 설명한다.
  //
  // 실제로 그렇게 됐다. #510 이 «반복 거래» 절을 더한 뒤 #519 가 문서를 전면
  // 재작성했는데, 두 변경이 **충돌 없이 자동 병합돼** 절이 그대로 둘 남았다.
  // 한쪽은 "홈에서 확인해야 생긴다", 다른 쪽은 "앱을 다시 열 때 생긴다" 였다.
  //
  // 충돌이 안 났다는 것과 문서가 멀쩡하다는 것은 별개다. 자동 병합은 텍스트가
  // 겹치지 않으면 통과시키고, 그 판단에 "같은 주제인가" 는 들어 있지 않다.
  it('같은 제목이 두 번 나오지 않는다', () => {
