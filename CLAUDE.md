...
실제로 19분 매달린 적이 있다 — 테스트가 띄운 서버가 안 죽어서 러너를 붙잡았다.

값의 근거: 가장 느린 테스트가 886ms, 전체 스위트가 5.4초다. 60초면 68배 여유라
CI 러너가 훨씬 느려도 정상 테스트를 끊지 않는다. 조이는 값이 아니라 **무한을
없애는 값**이다.

서버를 띄우는 테스트는 `try`/`finally` 로 감싸 **단언이 던져도 죽인다.** 안 그러면
살아남은 자식이 러너를 붙잡아 이 상한까지 기다리게 된다.
