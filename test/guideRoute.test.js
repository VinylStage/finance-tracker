...
const PORT = 20601; // 다른 테스트와 충돌 안 나게 임의 포트 사용
...
//
// 포트는 PORT + 1 로 만들지 않는다. 20601 + 1 은 savingsRoute 가 쓰는 20602 라
...
test('guide 라우트 — 문서가 없으면 404 이고 내부 경로를 흘리지 않는다', async () => {
  ...
  const alt = await startTestServer({ port: 20810, env: { GUIDE_PATH: missing } });
  try {
