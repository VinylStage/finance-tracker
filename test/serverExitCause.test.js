...
test('SIGTERM 을 받으면 원인을 stderr 로 남긴다', async () => {
  await withServer(21970, async (s) => {
    assert.ok(await waitReady(21970), '서버가 안 떴다');
    s.proc.kill('SIGTERM');
...
test('SIGINT 도 같은 방식으로 남는다', async () => {
  await withServer(21971, async (s) => {
    assert.ok(await waitReady(21971), '서버가 안 떴다');
    s.proc.kill('SIGINT');
    await waitExit(s.proc);
...
test('종료코드는 여전히 0 이다', async () => {
  await withServer(21972, async (s) => {
    assert.ok(await waitReady(21972), '서버가 안 떴다');
    s.proc.kill('SIGTERM');
...
test('그냥 뜨기만 하면 종료 메시지가 없다', async () => {
  await withServer(21973, async (s) => {
    assert.ok(await waitReady(21973), '서버가 안 떴다');
...
test('포트가 물려 있으면 헬퍼가 그 서버에 안 붙는다', async () => {
  await withServer(21974, async () => {
    assert.ok(await waitReady(21974), '앞선 서버가 안 떴다');
    await assert.rejects(
      () => startTestServer({ port: 21974 }),
      (e) => {
...
test('헬스 응답이 pid 를 준다', async () => {
  await withServer(21972, async (s) => {
    assert.ok(await waitReady(21972), '서버가 안 떴다');
    const r = await fetch('http://127.0.0.1:21972/api/health');
...
test('포트가 물리면 종료코드가 0 이 아니다', async () => {
  await withServer(21971, async () => {
    assert.ok(await waitReady(21971), '첫 서버가 안 떴다');
    await withServer(21971, async (second) => {
...
test('포트가 물리면 원인이 stderr 에 남는다', async () => {
  await withServer(21972, async () => {
    assert.ok(await waitReady(21972), '첫 서버가 안 떴다');
    await withServer(21972, async (second) => {
      // The error message format is "[server] 리스닝 실패: EADDRINUSE (127.0.0.1:21972)"
...
test('포트가 물리면 «기동 성공» 로그가 안 찍힌다', async () => {
  await withServer(21975, async () => {
    assert.ok(await waitReady(21975), '첫 서버가 안 떴다');
    await withServer(21975, async (second) => {
      // 출력을 읽기 전에 **종료를 기다린다.** 스폰 직후에는 아직 비어 있다.
      await waitExit(second.proc);
...
test('정상 경로에서는 헬퍼가 서버를 돌려준다', async () => {
  const server = await startTestServer({ port: 21973 });
  assert.ok(server.base.includes('21973'));
  server.stop();
});
