...
const TABLE_EXCLUDE = [
  ...
  // 018 이 만들고 025 가 지운다. 이 검사는 마이그레이션의 CREATE TABLE 만 훑어서
  // "만들어졌다가 지워진 표" 를 최종 스키마에 있는 것으로 본다. 지운 표를 문서에
  // 다시 적으라고 요구하는 셈이라 여기서 뺀다.
  //
  // 더 옳은 고침은 검사기가 DROP TABLE 까지 읽어 최종 스키마를 재구성하는 것이다.
  // 표 하나 때문에 검사기를 다시 쓰는 것은 이 PR 의 범위가 아니라고 봤다 —
  // 같은 상황이 또 생기면 그때 옮긴다.
  { re: /^accounts$/, why: '018 이 만들고 025 가 드롭한다 (계좌 기능 제거)' },
];
